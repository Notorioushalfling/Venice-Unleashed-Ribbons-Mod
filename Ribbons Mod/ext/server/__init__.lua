...
local ribbons = {
    ...
    AntiVehicleRibbon = { xp = 300, vehiclesDestroyed = 3 }, -- Adjusted XP to 300
...
function displayRibbon(player, ribbon)
    -- Check if WebUI is initialized before calling
    if WebUI then
...
-- Initialize the WebUI (Path needs to match the location of your WebUI folder and HTML file)
...
Events:Subscribe('Player:Score', function(player, scoringTypeData, score)
    print("A SCORING EVENT HAS OCCURED!!!!")
end)